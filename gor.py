...
class GraphOfRelations (textgraphs.SimpleGraph):
    """
...
Constructor.
        ...
        self.node_list: typing.List[ textgraphs.Node ] = []
        self.edge_list: typing.List[ textgraphs.Edge ] = []
...
Load data for a source graph, as illustrated in _InGram_
        ...
        with open(json_file, "r", encoding = "utf-8") as fp:  # pylint: disable=C0103,W0621
            ...
            for src_name, links in dat["ents"].items():
                src_node: textgraphs.Node = self.make_node(
                    [],
                    src_name,
                    None,
                ...
                for rel_name, dst_name in links:
                    ...
                    dst_node: textgraphs.Node = self.make_node(
                        [],
                        dst_name,
                        None,
                        textgraphs.NodeEnum.ENT,
                    ...
                    edge: textgraphs.Edge = self.make_edge(  # type: ignore  # pylint: disable=W0612,W0621
                        src_node,
                        dst_node,
...
if __name__ == "__main__":
    graph: GraphOfRelations = GraphOfRelations()
